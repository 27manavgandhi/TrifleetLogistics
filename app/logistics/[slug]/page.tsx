...
export function generateMetadata({ params }: Props): Metadata {
  ...
  return constructMetadata({
    ...
    description: `Tri Fleet Logistics provides full truck load, part truck load and B2B logistics services in ${city.name}, ${city.state}. ${city.blurb}`,
    ...
    <>
      ...
      <section className="section-py relative bg-secondary/40">
        <div className="container-px mx-auto max-w-8xl">
          <SectionHeading eyebrow="Local Advantage" title={`Why ${city.name} businesses choose Tri Fleet`} highlight="Our local presence translates into faster pickups, on-the-ground support and reliable delivery." />
      ...
      <section className="section-py relative">
        <div className="container-px mx-auto max-w-8xl">
          <SectionHeading eyebrow="Services" title={`Logistics services in ${city.name}`} highlight="The Tri Fleet services most relevant to businesses in and around the city." />
            ...
            {city.servicesHere.map((name) => {
              ...
              return (
                <StaggerItem key={name}>
                  {svc ? (
                    <Link href={`/services/${svc.slug}`} className="group flex items-center gap-4 rounded-2xl border border-border bg-card p-5 shadow-sm transition-all hover:-translate-y-1 hover:shadow-card">
