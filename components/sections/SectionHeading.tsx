...
export function SectionHeading({
  ...
  return (
    ...
    >
      ...
      {highlight && (
        <Reveal delay={0.1}>
          <p className={cn('max-w-3xl text-pretty text-base leading-relaxed sm:text-lg', variant === 'dark' ? 'text-white/70' : 'text-muted-foreground')}>
            {highlight}
          </p>
        </Reveal>
      )}
      {description && (
        <Reveal delay={0.15}>
          <p className={cn('max-w-2xl text-pretty text-sm leading-relaxed sm:text-base', variant === 'dark' ? 'text-white/60' : 'text-muted-foreground')}>
            {description}
          </p>
        </Reveal>
      )}
